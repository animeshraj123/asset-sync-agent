...
The decision-making core. Every tick, for every asset, the agent asks:
  ...
  1. Is this asset worth spending a query on right now?
     -> priority = decayed uncertainty x criticality. Below threshold, skip.
  2. Which sensor(s) first? -> Always cheapest first (telemetry, rfid_gate).
  3. Do the cheap readings agree? If not / nothing returned, cross-check
     with uwb_tracker before considering escalation.
  4. Is the remaining disagreement worth the expensive vision sensor?
     -> escalate only if expected_value(criticality, conflict_magnitude)
        exceeds its cost. Logged with the actual numbers every time.

Reconciliation is a recency- and reliability-weighted vote, not "trust the
newest" or "trust the most reliable sensor" — a stale reading from a
97%-reliable sensor can lose to a fresh reading from an 85%-reliable one.
"""
